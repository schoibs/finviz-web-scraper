...
def getScreenerPerformanceData(exchange: str, page:int = 1, market_cap='', sector='', index='', target_price='', p_e=''):
    ...
    # extract the website html content
    url = f'https://finviz.com/screener.ashx?v=141&f=exch_{exchange.lower()[:4]},cap_{market_cap},sec_{sector},idx_{index},targetprice_{target_price},fa_pe_{p_e}&r={page}'
    response = scraper.get(url)
    soup = BeautifulSoup(response.content, features="html.parser")
